...
if($_POST)
{
	 ...
	 * Tonjoo License
	 */
	if(class_exists('TonjooPluginLicenseSSLIDER'))
	{
		$PluginLicense = new TonjooPluginLicenseSSLIDER($_POST['sangar_slider']['license_key']);
	 ...
	 * Update options
	 */
	update_option('sangar_slider', $_POST['sangar_slider']);
...
$options = get_option('sangar_slider'); 
...
<div class="wrap">
	<?php echo "<h2>".__("Tonjoo License For Sangar Slider ",SANGAR_SLIDER)."</h2>"; ?>
	<p>
		<?php 
			_e('Register your license code here to get all benefit of Sangar Slider. ',SANGAR_SLIDER);
			_e('Find your license code at ',SANGAR_SLIDER);
		?>		
	...
	<form method="post" action="">
		<?php
			...
			if($license)
			{
				...
				if($license['status'])
				{
					...
					$license_status.= __('Registered',SANGAR_SLIDER);
					$license_status.= "</span>";
				} else {
					$license_status = "<span style='color:red'>";
					$license_status.= __($license['message'],SANGAR_SLIDER);
					$license_status.= "</span>";
				}
		...
		<tr valign="top" id="license_status">
			...
			<td style="width: 300px;" colspan="2">
				<input type="text" name="sangar_slider[license_key]" value="<?php echo $options['license_key'] ?>" style="width:300px;">
