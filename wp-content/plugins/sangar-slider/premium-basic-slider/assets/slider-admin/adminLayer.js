	...
	adminLayer = function(base)
	{
		...
		$('.fullscreen-ui-with-button-modal a.media-menu-item').on('click',function(){
			...
			switch($(this).attr('id'))
			{
				...
				case 'opt-mobile-tab':
					...
					var video_bg = $(element).find('#opt-background [tab-bg-video-html5-poster] input[media-upload-image-url]').val();
					var color = $(element).find('#opt-background input[name="tab-bg-color"]').val();
					var mobileBgEnabled = $(element).find('#opt-background-mobile select[name="tab-bg-mobile-enabled"]').val();
					var mobileBg = $(element).find('#opt-background-mobile [tab-bg-mobile-image] input[media-upload-image-url]').val();

					// if mobile background is enabled
					if(mobileBgEnabled == 'true') {
						bg = mobileBg;
					}
		...
		$('#layer-modal-image').dialog({
			...
			buttons: {
		        ...
		        'Apply': function(){
		        	...
		        	var data = {
		        		...
		        		contentType: 'image',
		        		animation: $(this).find('#add-layer-single-animation').val(),
		        		others: {
		    ...
		    open:function () {
				...
				$(this).find('#add-layer-single-animation').val(data.animation);
		...
		$('#layer-modal-text').dialog({
			...
			buttons: {
		        ...
		        'Apply': function(){
					...
					var others = {
						...
						text: tonjooWpEditor.getContent('add-layer-text'),
	        			...
	        			text_decoration: $(this).find('select[name = add-layer-text-decoration]').val(),
	        			font_style: $(this).find('select[name = add-layer-text-font-style]').val(),
	        			...
	        			border_position: $(this).find('select[name = add-layer-text-border-position]').val(),
	        		...
	        		if(others.border_size != '')
	        			css+= others.border_position + ": " + others.border_size + 'px solid ' + others.border_color + ';';
					...
					if(others.text_decoration != '')
						p_css+= "text-decoration: " + others.text_decoration + ";";

					if(others.font_style != '')
						p_css+= "font-style: " + others.font_style + ";";
					...
					// AJAX DO SHORTCODE HERE
					// var data_ajax = {
					// 	action: 'sslider_do_shortcode_content',
					// 	content: others.text
					// }
					...
					// $.post(ajaxurl, data_ajax, function(response){
						...
						response = others.text;

						// cleaning text
						var content_text = response.replace(/<p[^>]*>/g, '').replace(/<\/p>/g, '<br />');

		        		// print html
						var html = "<div style='" + css + "' ><span style='" + p_css + "'>" + content_text + "</span></div>";
			        	...
			        	var hyperlink = thisDialog.find('#add-layer-text-hyperlink');
							hyperlink = hyperlink.val();

						var hyperlinkTarget = thisDialog.find('#add-layer-text-hyperlink-target');
							hyperlinkTarget = hyperlinkTarget.val();
			        	...
			        	var data = {
			        		...
			        		hyperlinkTarget: hyperlinkTarget,
			        		contentType: 'text',
			        		animation: thisDialog.find('#add-layer-single-animation').val(),
			        		others: others
			        	}
		    ...
		    open:function () {
		        ...
		        // set the WYSIWYG
		        tonjooWpEditor.modalOpen('add-layer-text');
		        ...
		        $(this).find('select[name = add-layer-text-border-position]').val(others.border_position);
		        $(this).find('#add-layer-text-border-size').val(others.border_size);
		        $(this).find('#add-layer-text-border-color').minicolors('value',others.border_color);
		        ...
		        $(this).find('select[name = add-layer-text-decoration]').val(others.text_decoration);
		        $(this).find('select[name = add-layer-text-font-style]').val(others.font_style);
		    ...
		    close:function() {
		    	// set the WYSIWYG
		        tonjooWpEditor.modalClose('add-layer-text');
		    }
	    }).parent('.ui-dialog').attr('id','sangar-slider-modal-dialog');
		...
		$('#layer-modal-html').dialog({
			...
			buttons: {
		        ...
		        'Apply': function(){
	        		...
	        		// comment this to let him to be empty
