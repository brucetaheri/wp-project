...
function sslider_admin_footer()
{
    ...
    require_once( plugin_dir_path( __FILE__ ) . 'modal-add-layer.php');
}
...
function sslider_admin_enqueue_scripts($hook)
{
    ...
    // load the font for layer editor, if not at the preview session
    if($hook == 'post.php') {
        global $post;
        ...
        if($config['is_preview'] != 'true') {
            require_once( plugin_dir_path( __FILE__ ) . 'load-layer-fonts.php');
            
            $layer_fonts = new ssliderLoadLayerFonts($post->ID);
            $layer_fonts->print_fonts();
        }
    }
}