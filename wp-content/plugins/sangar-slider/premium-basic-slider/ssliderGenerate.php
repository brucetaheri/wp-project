...
Class ssliderGenerateAddonBasic extends ssliderGenerate
{
	...
	function html()
	{
		...
		{		
			...
			$html.= "<div class='sangar-content' style='display:none;' advance-speed='{$value['slide-advanceSpeed']}'>";
			...
			if($value['slide-type'] == 'static')
			{
				...
				// put empty layer data to js
				$put_to_javascript.= 'sslider_layer_data["' . $this->slider_id . '"][' . $iterate_num . '] = false;';
			}
			else if($value['slide-type'] == 'layer')
			{
				...
				$php_content = json_decode($value['slide-layer']);

				// allowed layer content type to be shortcode-able
				$shortcode_content_type = array('text', 'html');
				...
				if(isset($php_content->desktop->content) && is_array($php_content->desktop->content)) {
					...
					for ($i=0; $i < count($content_desktop); $i++) {
						if(! isset($content_desktop[$i]->contentType)) continue;
						
						if(in_array($content_desktop[$i]->contentType, $shortcode_content_type)) {
	...
	private function slide_background($slide)
	{
		...
		// background mobile
		if($slide['tab-bg-mobile-enabled'] == 'true')
		{
			$attr = array(
				'class' => 'sangar-bg-mobile',
				'alt' => $slide['slide-title']
			);

			$background_mobile = wp_get_attachment_image($slide['tab-bg-mobile-image'],'full',false,$attr);
